...
def plotclusters3D(data, labels, peaks):
    ...
    for idx, peak in enumerate(rgb_peaks):
        ...
        ## uncomment for plotting peaks for the debugging data
        # ax.scatter(peak[0], peak[1], peak[2], c='black', s=100.5, marker='x')
        ax.scatter(cluster[0], cluster[1], cluster[2], s=.3, label="Cluster " + str(idx))
